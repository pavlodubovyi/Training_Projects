...
while True:
    ...
    if wait_for_number: # не виконується, якщо False - весь блок, аж до else (там де пішли +-*/)
        try:
        ...
        if not result: # цей блок виконується ТІЛЬКИ в перший раз, бо if not None = True. Якщо if not True, це False і блок до наступного if не виконується
            result = operand
        ...
        if user_input in ["+", "-", "*", "/"]:
            operator = user_input
            wait_for_number = True # і знову запускаємо цикл на цифри, пропускаємо на math
        else:
